...
d <- nclazy %>% 
  dplyr::filter(AREA > .1) %>% 
  dplyr::select(AREA, PERIMETER, BIR74, FIPS, geometry) %>% 
  collect()
d$geometry <- geojsonsf::geojson_sfc(d$geometry)
d %>%   sf::st_as_sf() %>% 
  plot()
...
dtbl(con, "nc.gpkg") %>% filter(between(AREA, 0.05, 1))
...
## plus no-geometry causes problems
